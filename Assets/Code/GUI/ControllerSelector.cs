...
namespace TAMKShooter.GUI
{
    public class ControllerSelector : MonoBehaviour
    {
        ...
        public void Init(PlayerData.PlayerId id, InputManager.ControllerType defaultControllerType)
        {
            ...
            foreach(var value in Enum.GetValues(typeof(InputManager.ControllerType)))
            {
                if((InputManager.ControllerType)value != InputManager.ControllerType.Error)
                {
                    string controllerName = InputManager.GetControllerName((InputManager.ControllerType)value);
